...
public class HomePage implements Screen {
    ...
    public void show() {                      // show is create() for screens(analogy)
        ...
        logout_image = new Texture("player_logout_button.png");
        logout_button = new ImageButton(new TextureRegionDrawable(logout_image));
        logout_button.setPosition(535, 270);
        logout_button.setSize(100, 100);

        stage.addActor(play_button);
        stage.addActor(quit_button);
        stage.addActor(logout_button);
    ...
    public void render(float delta) {
        ...
        if (quit_button.isPressed()) {
            Player.store_players_to_file();
            // Quit Application
            Gdx.app.exit();
        }

        if (logout_button.isPressed()) {                                // logout button functionality
            Player.store_players_to_file();
