...
//    public ArrayList<Pig> pigs = new ArrayList<>();
    ...
    public Level2(MainGame game, Player player, Slingshot slingshot) {
    ...
    public Level2(MainGame game, Player player, Slingshot slingshot, World w, boolean restored) {
        this.game = game;
        this.current_player = player;
        this.slingshot = slingshot;
...
//
        ...
        if (bird != null) {
            ...
            game.setScreen(new LostScreen(game, current_player, slingshot, 2));
            return;
        }
    ...
    public void render(float delta) {
        ...
        if (Gdx.input.isKeyJustPressed(Input.Keys.ENTER)) {
            ...
            if(current_player.getLevels_cleared()<2){
                current_player.setLevels_cleared(2);
            }
            game.setScreen(new VictoryScreen(game, current_player, slingshot, 2));
            return;
        }
    ...
    public void setPigs() {
        this.pigs.clear();
        this.pigs.add(new Pig("Level_1images/kingpig.png", 15));
        this.pigs.add(new Pig("taupig.png",     12));
        this.pigs.add(new Pig("helmetpig.png", 18));

        temp_pig = pigs.getFirst(); temp_pig.create(world, 440, 255);
        temp_pig = pigs.get(1); temp_pig.create(world, 520, 255);
        temp_pig = pigs.get(2); temp_pig.create(world, 600, 255);
    ...
    private void setBlocks() {
        for(int i=1 ; i<=5; ++i ){
            ...
            for(int j =0; j<randomNumber; ++j){
                switch(i){
                    case 1:this.blocks.add(new Block("Level_1images/woodblock.png",3)); break;
                    case 2:this.blocks.add(new Block("iceblock.png",4));break;
                    case 3:this.blocks.add(new Block("steelblock.png",5));break;
                    case 4:this.blocks.add(new Block("iceblock.png",4));break;
                    case 5:this.blocks.add(new Block("Level_1images/woodblock.png",3)); break;
                    default:break;
                }
                temp_block=blocks.getLast(); temp_block.create(world, (400+(40)*i), (55+(40)*j), 35, 35);
            }
        }
    }
