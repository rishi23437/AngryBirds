...
//    public ArrayList<Pig> pigs = new ArrayList<>();
    ...
    public Level1(MainGame game, Player player, Slingshot slingshot, World w, boolean restored) {
        this.game = game;
        this.current_player = player;
        this.slingshot = slingshot;
...
//
        ...
        if (bird != null) {
            ...
            game.setScreen(new LostScreen(game, current_player, slingshot, 1));
            return;
        }
    ...
    public void render(float delta) {
        ...
        if (Gdx.input.isKeyJustPressed(Input.Keys.ENTER)) {
            ...
            if(current_player.getLevels_cleared()<1){
                current_player.setLevels_cleared(1);
            }
            game.setScreen(new VictoryScreen(game, current_player, slingshot, 1));
            return;
        }
    ...
    public void setPigs() {
        ...
        temp_pig = pigs.getFirst(); temp_pig.create(world, 500, 125);
        temp_pig = pigs.get(1); temp_pig.create(world, 535, 90);
        temp_pig = pigs.get(2); temp_pig.create(world, 570, 125);

        num_pigs_remaining = pigs.size();
    }

    private void setBlocks() {
        this.blocks.add(new Block("Level_1images/woodblock.png",3));
        this.blocks.add(new Block("steelblock.png",3));
        this.blocks.add(new Block("iceblock.png",3));
        this.blocks.add(new Block("Level_1images/woodblock.png",3));
        this.blocks.add(new Block("iceblock.png",3));
